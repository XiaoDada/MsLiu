...
public class ScrollLayout extends FrameLayout {
    ...
    private OrcWebView mOrcWebView;
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.ScrollLayout_ScrollLayoutMode)) {
            int mode = a.getInteger(R.styleable.ScrollLayout_ScrollLayoutMode, 0);
            switch (mode) {
                case 0x0:
                    setToOpen();
                    break;
    ...
    OrcWebView.OnScrollChangeListener OnWebViewScrollChangeListener = new OrcWebView.OnScrollChangeListener() {
        @Override
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (currentInnerStatus == InnerStatus.SCROLLING) {
            return false;
        }
    ...
    public void scrollToOpen() {
        ...
        if (currentInnerStatus == InnerStatus.OPENED || currentInnerStatus == InnerStatus.SCROLLING) {
            return;
        }
        if (maxOffset == minOffset) {
            return;
...
//        if (!isSupportExit) return;
        if (currentInnerStatus == InnerStatus.EXIT) {
//            Log.e("fengda","currentInnerStatus == InnerStatus.EXIT");
            return;
        }
        if (exitOffset == maxOffset) {
//            Log.e("fengda","exitOffset == maxOffset");
            return;
        }
...
//        Log.e("fengda", "dy == 0" + duration);
        scroller.startScroll(0, getScrollY(), 0, dy, 200);
//        Log.e("fengda", "getScrollY()" + getScrollY() + "====" + dy);
//        invalidate();
        postInvalidate();
    }
...
//        if (!isSupportExit) return;
    ...
    public void setAssociatedWebView(OrcWebView orcWebView) {
        this.mOrcWebView = orcWebView;
//        this.mOrcWebView.setScrollbarFadingEnabled(false);
        this.mOrcWebView.setOnScrollChangeListener(OnWebViewScrollChangeListener);
    }
